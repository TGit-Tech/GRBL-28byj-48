...
// SPINDLE_ENABLE_PORT ^= 1<<SPINDLE_ENABLE_BIT; // Debug: Used to time ISR
  ...
  if (st.counter_z > st.exec_block->step_event_count) {
    ...
    if (st.exec_block->direction_bits & (1<<Z_DIRECTION_BIT)) { sys.position[Z_AXIS]--;
      ...
      if (costyz==1)   PORTB=0B1000;
      if (costyz==2)   PORTB=0B1100;
      if (costyz==3)   PORTB=0B0100;
      if (costyz==4)   PORTB=0B0110;
      if (costyz==5)  PORTB=0B0010;
      if (costyz==6)  PORTB=0B0011;
      if (costyz==7)  PORTB=0B0001;
      if (costyz==8) PORTB=0B1001;
